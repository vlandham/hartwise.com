jQuery(function($){
    ...
    if($.browser.mozilla){
        // Fixes a Firefox issue
        $('#mainmenu ul#mainmenu-menu > li > ul.sub-menu > li').each(function(){
            $(this).parent().closest('.menu-item').css('position', 'static'); // Reset positioning
            $(this).closest('.sub-menu').css({
                'width': $(this).parent().closest('li').width(),
                'left' : $(this).parent().closest('li').position().left
            });
        });
    }
