...
function siteorigin_settings_admin_menu() {
	$theme = wp_get_theme();
	$page = add_theme_page(
		sprintf(__( '%s Theme Settings', 'pitch' ), $theme->get('Name')),
		sprintf(__( 'Theme Settings', 'pitch' ), $theme->get('Name')),
		'edit_theme_options',
		'theme_settings_page',
...
function siteorigin_settings_render() {
	if( version_compare( get_bloginfo('version'), '3.4', '<' ) ) {
		?><div class="wrap"><div id="setting-error-settings_updated" class="updated settings-error"> <p><strong><?php _e('Please update to the latest version of WordPress to use theme settings.', 'pitch') ?></strong></p></div></div><?php
		return;
	}
...
function siteorigin_setting_editable_option_field(){
	siteorigin_settings_add_field('general', 'hover_edit', 'checkbox', __('Display Hover Edit Icon', 'pitch'), array(
		'description' => __('Display a small icon that makes quickly editing fields easy. This is only shown to admin users.', 'pitch'),
	));
}
...
function siteorigin_settings_field( $args ) {
	...
	switch ( $args['type'] ) {
		case 'checkbox' :
			...
			<label for="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_attr( !empty( $args['label'] ) ? $args['label'] : __( 'Enabled', 'pitch' ) ) ?></label>
			<?php
			break;
		case 'text' :
		case 'number' :
		...
		case 'media':
			if(version_compare(get_bloginfo('version'), '3.5', '<')){
				printf(__('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'pitch'), admin_url('update-core.php'));
				break;
			}
			...
			$choose_title = empty($args['choose']) ? __('Choose Media', 'pitch') : $args['choose'];
			$update_button = empty($args['update']) ? __('Set Media', 'pitch') : $args['update'];
			...
			?>
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php _e('Remove', 'pitch') ?></a>
				</div>
		...
		case 'teaser' :
			...
			<a class="premium-teaser siteorigin-premium-teaser" href="<?php echo admin_url( 'themes.php?page=premium_upgrade' ) ?>" target="_blank">
				<em></em>
				<?php printf( __( 'Only available in <strong>%s Premium</strong> - <strong class="upgrade">Upgrade Now</strong>', 'pitch' ), ucfirst($theme) ) ?>
		...
		case 'gallery' :
			...
			<input
				...
				type="text"
				value="<?php echo esc_attr( $current ) ?>" />
			<a href="#" class="so-settings-gallery-edit"><?php _e('Select Images', 'pitch') ?></a>
			<?php
			break;
		...
		case 'pages' :
			...
			?>
			<select id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>">
				<option value="0"><?php _e('None', 'pitch') ?></option>
...
function siteorigin_settings_change_message() {
	if ( get_theme_mod( 'siteorigin_settings_changed' ) ) {
		...
		<div id="setting-updated" class="updated">
			<p>
				<strong><?php _e( 'Settings saved.', 'pitch' ) ?></strong>
			</p>
		</div>
		<?php
...
function siteorigin_settings_theme_help(){
	...
	$text = sprintf(
		__( "Read %s's <a href='%s'>theme documentation</a> for help with these settings.", 'pitch' ),
		ucfirst($theme_name),
