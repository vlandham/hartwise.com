			...
			<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>    
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>			
					...
					if ( $portfolio_layout == 'standard' ) { 
						echo cudazi_get_featured_image( 'grid_8', '' );
					 } else { 
						echo cudazi_get_featured_image( 'grid_12', '' );
					 }
					 */ 
					 ?>
		...
		<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
			<div class="grid_4" id="primary">
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>								
					...
					<div class="entry-content">
						<?php the_content(); ?>
						<?php edit_post_link( __('Edit Portfolio Item','cudazi') ); ?>
					</div><!--//entry-content -->     				
				</div><!-- //post-->
			</div><!--//grid_X-->
			<div class="grid_8" id="aside">										
				<?php
					...
					if ( $portfolio_layout == 'attachment_list' || ! $portfolio_layout ) {
						...
						$cudazi_gallery_options = array( 'display' => 'list', 'source' => 'attachment' );					
						echo cudazi_attachment_gallery( $cudazi_gallery_options );	
						
					}else if ($portfolio_layout == 'attachment_slider' ){
						
						$cudazi_gallery_options = array( 'display' => 'slider', 'source' => 'attachment' );					
						echo cudazi_attachment_gallery( $cudazi_gallery_options );	
